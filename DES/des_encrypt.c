...
void convert_ascii_to_char(char* ciphertext_bits, char* ciphertext);
...
int main()
{
	char plaintext[100],ciphertext[100],key[10];
	printf("Enter plain text(max 100 characters)to encrypt: ");
	scanf("%s",plaintext);
	printf("Enter key (of 8 characters) for encryption: " );
	scanf("%s",key);
	char plaintext_bits[1000];
	char* KEY = (char*)malloc(sizeof(char)*65);
	convert_char_to_ascii(plaintext,plaintext_bits);
	...
	block[64]='\0';
	int j;
	for(i=0;i<strlen(plaintext_bits);i+=64){
		for(j=0;j<64;j++){
			if(i+j<strlen(plaintext_bits))
				block[j] = plaintext_bits[i+j];
		}
		block[64] = '\0';
		block = encrypt(block,KEY);
		for(j=0;j<64;j++){
			ciphertext_bits[i+j] = block[j];
		}
	}
	ciphertext_bits[i] = '\0';
	printf("cipher text bits = %s\n",ciphertext_bits);
	convert_ascii_to_char(ciphertext_bits,ciphertext);
...
void convert_ascii_to_char(char* ciphertext_bits, char* ciphertext)
{
	int i,len=strlen(ciphertext_bits),j=0,k=0;
	for(i=0;i<len;i+=8){
		int ascii = 0;
		for(j=0;j<8;j++){
			ascii += (int)(ciphertext_bits[i+j]-'0')*pow(2,(7-j));
		}
		ciphertext[k++] = (char)ascii;
	}
	ciphertext[k] = '\0';
}
