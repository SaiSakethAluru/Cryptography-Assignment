...
int** get_next_key(int** key_matrix);
...
// int round_constant = {0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36}
...
int main()
{
	...
	// encrypt(plaintext_matrix,key_matrix);
	ciphertext_matrix = add_round_key(plaintext_matrix,key_matrix);
	key_matrix = get_next_key(key_matrix);
	...
	for(i=0;i<4;i++){
		for(j=0;j<4;j++){
			printf("%d ",ciphertext_matrix[i][j]);
		}
		printf("\n");
	}
	return 0;
...
// void encrypt(plaintext_matrix,key_matrix)
// {
// 	int** ciphertext_matrix;
// 	ciphertext_matrix = (int**)malloc(sizeof(int*)*4);
// 	ciphertext_matrix[i] = (int*) malloc(sizeof(int)*4);
...
int** mix_column(int** matrix)
{
	...
	for(i=0;i<4;i++){
		for(j=0;j<4;j++){
			int two_mul = matrix[j%4][i]<<1;
			if(matrix[j%4][i]/128 == 1)
				two_mul = two_mul ^ 27;
			int three_mul = matrix[(j+1)%4][i]<<1;
	...
	return new_matrix;
		// two_mul = matrix[1][i]<<1;
		// if(matrix[1][i]/128 == 1)
			// two_mul = two_mul ^27;
		// three_mul = matrix[2][i] <<1;
		// if(matrix[2][i]/128 == 1)
		// 	three_mul = three_mul ^ 27;
		// three_mul = three_mul ^ matrix[2][i];
		// new_matrix[1][i] = matrix[0][i] ^ two_mul ^ three_mul ^ matrix[3][i];
}