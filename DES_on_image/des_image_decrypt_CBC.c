...
int main()
{
	...
	IplImage* source_img = cvLoadImage("CipherImage_CBC.png",0);
	char key[9],IV[9];
	...
	char* ciphertext_previous = (char*)malloc(sizeof(char)*65);
	convert_char_to_ascii(key,KEY);
	convert_char_to_ascii(IV,ciphertext_previous);
	if (!source_img)
	{
	...
	cvNamedWindow("ciphertext",CV_WINDOW_AUTOSIZE);
	cvShowImage("ciphertext",img);
	cvWaitKey(0);
	IplImage* plain = cvCreateImage(cvSize((int)(img->height),(int)(img->width)),img->depth,img->nChannels);
	IplImage* tmp = cvCreateImage(cvSize((int)(img->height),(int)(img->width)),img->depth,img->nChannels);
	int rows,cols,i,j,k;
	uchar* data = img->imageData;
	rows = img->height;
	cols = img->width;
	int* plain_pixels = (int*)malloc(sizeof(int)*8);
	char* ciphertext_bits = (char*)malloc(sizeof(char)*65);
	...
	for(i=0;i<rows;i++){
		for(j=0;j<cols;j+=8){
			for(k=0;k<8;k++){
				...
				int x;
				for(x=0;x<8;x++){
					ciphertext_bits[k*8+x] = pixel_bits[x];
				}
			}
			ciphertext_bits[64] = '\0';
			block = decrypt(ciphertext_bits,KEY);
			block = xor(block, ciphertext_previous, 64);
			strcpy(ciphertext_previous,ciphertext_bits);
			plain_pixels = convert_bits_to_pixels(block);
			uchar* ptr = (uchar*) (plain->imageData + i*plain->widthStep);
			for(k=0;k<8;k++){
				ptr[j+k] = plain_pixels[k];
			}
		}
	}
	cvNamedWindow("temp",CV_WINDOW_AUTOSIZE);
	cvShowImage("temp",tmp);
	cvNamedWindow("plain",CV_WINDOW_AUTOSIZE);
	cvShowImage("plain",plain);
	cvWaitKey(0);
	cvSaveImage("PlainImage.jpg",plain,0);
	cvReleaseImage(&plain);
	cvDestroyWindow("plain");
	return 0;
}
...
// convert 64-bit plain text string to array of 8 bit pixel values
int* convert_bits_to_pixels(char* bits)
{
	int i,j;
